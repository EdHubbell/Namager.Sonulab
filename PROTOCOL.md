...
| `dswap`  | `dswap <path>:{"index":A,"index2":B}\0` | **CONFIRMED live 2026-07-24 (fw 2.5.1, serial, `--dswap-probe`)**: atomically swaps two slots — name AND content, byte-verified — in **~120–235 ms**; ACK echoes the command. Undocumented in VoidX UI; found via `app.so` string pool (`dswap `, `,"index2":`). ~7× faster than one select+save reorder step. **Works on all three blocks** (re-probed 2026-07-24, `--dswap-probe --path`): `root\presets` ~200 ms, `root\amp` ~235 ms, `root\ir` ~120 ms — each a full name+content swap, self-inverse (a second identical `dswap` restores), `restored=True`. **Active-slot safe:** swapping the slot of the CURRENTLY-SELECTED (live) preset does NOT disturb the live preset (`--dswap-probe --path root\presets --active`: `live preset UNDISTURBED`) — so a reorder engine needs no post-swap re-select. `root\amp`/`root\ir` support unlocks amp/IR reorder (Cycle 2). |
