...
- **Existing tests must keep passing.** Baseline is 490 tests via `dotnet test`. New optional constructor parameters must default to the existing no-op fallbacks (`NullStatusService.Instance`, `NullPresetUsageService.Instance`).
...
```csharp
    [Fact] public void EnsureTabLoaded_zero_refreshes_the_editor_options_when_an_editor_exists()
    {
        var vm = new MainWindowViewModel();
        vm.EnsureTabLoaded(0);                   // no connection yet: must not throw, must not set a task
        Assert.Null(vm.Editor);
    }
```

This is a guard test only — the connected path needs a live `DeviceSession` and is covered by the manual hardware checklist. Keep it minimal and honest.

- [ ] **Step 2: Run the test to verify it passes for the wrong reason, then implement**

Run: `dotnet test tests/Namager.App.Tests --filter "FullyQualifiedName~MainWindowViewModelTests"`
Expected: PASS (the guard already holds). Implement Step 3 and re-run to confirm it still passes — this test exists to catch a null-deref regression in the new branch.
...
public partial class SlotPickerDialog : Window
{
    ...
    public SlotPickerDialog() => InitializeComponent();
    ...
    public static async Task<int?> ShowAsync(Window owner, IReadOnlyList<PresetItemViewModel> items)
    {
        ...
        foreach (var i in items.Where(i => !i.IsEmpty))
        {
            ...
            ((List<string>)dlg.SlotCombo.ItemsSource!).Add($"{i.DisplaySlot:00}  {i.Name}");
        }
...
`SlotCombo.ItemsSource` starts null, so initialise it in the constructor instead of casting:
...
```csharp
    ...
    {
        InitializeComponent();
        SlotCombo.ItemsSource = _labels;
    }
```

…and in `ShowAsync` use `dlg._labels.Add(...)`.
...
```csharp
    [Fact] public void SetTheme_updates_the_selected_flags()
    {
        var vm = new MainWindowViewModel();
...
- [ ] **Step 3: Add the theme members to the VM**
...
```csharp
    ...
    private void SetTheme(string? theme)
    {
        ...
        Namager.App.Services.AppSettingsStore.Save(new Namager.App.Services.AppSettings { Theme = theme });
    }
```

At the end of the constructor, before `Status.SetIdleSummary("Not connected");`, load the persisted value so the menu shows the right radio without re-saving:

```csharp
        _theme = Namager.App.Services.AppSettingsStore.Load().Theme;
```

- [ ] **Step 4: Add the menu to MainWindow**
...
- [ ] **Step 5: Create the About dialog**
...
- [ ] **Step 7: Build, test, and verify visually**
